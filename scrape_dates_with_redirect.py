#!/usr/bin/env python3
"""
Scrape actual meeting dates from Granicus agenda pages with redirect handling
"""
...
def scrape_meeting_dates_with_redirect():
    """Scrape actual meeting dates from Granicus agenda pages handling redirects"""
    ...
    for meeting_id, meeting_data in meetings_2024:
        ...
        try:
            # Make request to agenda page with redirect handling
            session = requests.Session()
            session.max_redirects = 5  # Allow up to 5 redirects

            response = session.get(agenda_url, timeout=15, allow_redirects=True)
            ...
            # Method 2: Look for date in page content (like "DECEMBER 19, 2023")
            if not date_found:
            ...
            # Method 3: Look for specific agenda header elements
            if not date_found:
                # Look for elements that might contain meeting dates
                header_elements = soup.find_all(['h1', 'h2', 'h3', 'div', 'span'], string=re.compile(r'(JANUARY|FEBRUARY|MARCH|APRIL|MAY|JUNE|JULY|AUGUST|SEPTEMBER|OCTOBER|NOVEMBER|DECEMBER).*\d{4}', re.I))
                for element in header_elements:
                    text = element.get_text().strip()
                    print(f"  Potential date element: {text}")
                    if re.search(r'(JANUARY|FEBRUARY|MARCH|APRIL|MAY|JUNE|JULY|AUGUST|SEPTEMBER|OCTOBER|NOVEMBER|DECEMBER).*\d{4}', text, re.I):
    ...
    with open('scraped_actual_2024_dates.json', 'w') as f:
        json.dump(scraped_dates, f, indent=2)

    print(f"\n💾 Scraped dates saved to 'scraped_actual_2024_dates.json'")
    print(f"   - Review the dates and update the main data file if they look correct")

if __name__ == "__main__":
    scrape_meeting_dates_with_redirect()
