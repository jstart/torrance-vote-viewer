#!/usr/bin/env python3
"""
Ultra aggressive deduplication to remove all remaining duplicates
"""
...
def ultra_normalize_agenda_item(agenda_item):
    """Ultra aggressive normalization for agenda items"""
    if not agenda_item:
        return ""
    ...
    # Remove common phrases that create duplicates
    ...
    normalized = re.sub(r'^\d+[a-z]?\s*\.?\s*', '', normalized)  # Remove item numbers
    normalized = re.sub(r'^[a-z]\s*\.?\s*', '', normalized)  # Remove single letters
    normalized = re.sub(r'\s*\(.*?\)\s*', ' ', normalized)  # Remove parenthetical content
    normalized = re.sub(r'\s*expenditure:.*$', '', normalized)  # Remove expenditure info
    normalized = re.sub(r'\s*$', '', normalized)  # Remove trailing spaces

    # Normalize common generic terms
    normalized = re.sub(r'^consent calendar.*$', 'consent calendar', normalized)
    normalized = re.sub(r'^administrative matters.*$', 'administrative matters', normalized)
    normalized = re.sub(r'^oral communications.*$', 'oral communications', normalized)
    normalized = re.sub(r'^council committee meetings.*$', 'council committee meetings', normalized)
    normalized = re.sub(r'^hearings.*$', 'hearings', normalized)
    normalized = re.sub(r'^adjournment.*$', 'adjournment', normalized)

    # Remove very short or generic items
    if len(normalized) < 3:
        normalized = "generic item"
...
def ultra_deduplication():
    ...
    # Group votes by meeting_id and ultra-normalized agenda item
    vote_groups = defaultdict(list)

    for vote in data['votes']:
        meeting_id = vote.get('meeting_id', '')
        agenda_item = vote.get('agenda_item', '')
        normalized_agenda = ultra_normalize_agenda_item(agenda_item)
