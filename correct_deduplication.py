#!/usr/bin/env python3
"""
Correct deduplication - only remove duplicates WITHIN the same meeting
"""
...
def normalize_agenda_item(agenda_item):
    """Normalize agenda item text for better matching within same meeting"""
    if not agenda_item:
        return ""
...
def correct_deduplication():
    ...
    # Group votes by meeting_id ONLY (not across meetings)
    ...
    for vote in data['votes']:
        ...
        # Create a key for grouping - ONLY within same meeting
    ...
    print(f"\n✅ Correct deduplication complete!")
    ...
    # Show recurring vote distribution
    print(f"\n📊 Recurring vote distribution by meeting:")
    recurring_votes = [vote for vote in data['votes'] if vote.get('agenda_item') and isinstance(vote['agenda_item'], str) and any(term in vote['agenda_item'].upper() for term in ['CONSENT CALENDAR', 'ADJOURNMENT', 'ADMINISTRATIVE MATTERS', 'ORAL COMMUNICATIONS', 'COUNCIL COMMITTEE MEETINGS', 'HEARINGS'])]
    recurring_by_meeting = defaultdict(int)
    for vote in recurring_votes:
        recurring_by_meeting[vote['meeting_id']] += 1

    for meeting_id in sorted(recurring_by_meeting.keys()):
        print(f"  Meeting {meeting_id}: {recurring_by_meeting[meeting_id]} recurring votes")

if __name__ == "__main__":
    correct_deduplication()
