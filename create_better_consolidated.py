#!/usr/bin/env python3
"""
Script to create a better consolidated dataset that preserves more votes
...
def create_better_consolidated_data():
    """Create a consolidated dataset that preserves more votes."""
    ...
    # Process original votes with smarter deduplication
    processed_votes = []
    seen_agenda_items = set()
    ...
    for vote in original_data['all_votes']:
        ...
        # Only skip if we've seen this exact agenda item before
        # This is less aggressive than frame-based deduplication
        if agenda_key in seen_agenda_items:
            continue
        
        seen_agenda_items.add(agenda_key)
    ...
    output_file = 'data/torrance_votes_better_consolidated.json'
    with open(output_file, 'w') as f:
        json.dump(consolidated_data, f, indent=2)
    
    print(f"\n✅ Created better consolidated data: {output_file}")
    print(f"📊 Final stats:")
...
if __name__ == "__main__":
    create_better_consolidated_data()
