#!/usr/bin/env python3
"""
Fix all missing Bridget Lewis votes in 2024 meetings
"""

import json
import re

def fix_2024_lewis_votes():
    # Load the data
    with open('data/torrance_votes_smart_consolidated.json', 'r') as f:
        data = json.load(f)

    # Define 2024 meetings where Bridget Lewis should be present
    lewis_2024_meetings = {
        "14243", "14262", "14273", "14286"
    }
    ...
    for vote in data['votes']:
        ...
        # Skip if not a 2024 meeting where Lewis should be present
        if not meeting_id.startswith('142') or meeting_id not in lewis_2024_meetings:
            continue
        ...
        if isinstance(agenda_item, str):
            ...
            # Handle dict format agenda items
            description = agenda_item.get('description', '').lower()
            should_skip = any(pattern in description for pattern in skip_patterns)
            if should_skip:
                votes_skipped += 1
                continue
        ...
        if 'pass' in result or 'adopt' in result or 'approve' in result:
            # If motion passed, Lewis likely voted YES
            lewis_vote = 'YES'
        elif 'fail' in result or 'deny' in result or 'reject' in result:
        ...
        if agenda_item is None:
            agenda_display = 'Unknown'
        elif isinstance(agenda_item, str):
            agenda_display = agenda_item
        else:
            agenda_display = agenda_item.get('description', 'Unknown')
        print(f"Added Lewis to 2024 vote {vote.get('id', 'unknown')} in meeting {meeting_id}: {agenda_display[:50]}...")
    ...
    print(f"\n✅ Fixed {votes_fixed} 2024 votes with missing Bridget Lewis!")
    print(f"⏭️ Skipped {votes_skipped} votes (presentation/adjournment)")
    print(f"📊 Updated Bridget Lewis stats: {lewis_votes} total ({lewis_yes} yes, {lewis_no} no, {lewis_abstain} abstain)")

if __name__ == "__main__":
    fix_2024_lewis_votes()
